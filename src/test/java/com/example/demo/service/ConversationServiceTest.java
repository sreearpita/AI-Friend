...
class ConversationServiceTest {
    ...
    @Test
    void createPersistsConversation() {
        AppUser user = new AppUser(UUID.randomUUID());
        when(conversationRepository.save(any(Conversation.class)))
                .thenAnswer(inv -> inv.getArgument(0));

        Conversation result = conversationService.create(user);
    ...
    void getRecentMessagesReturnsChronologicalOrder() {
        ...
        // Repository returns DESC order (newest first): m1 is newer ("second"), m2 is older ("first")
